...
echo $NS_DOMAIN > /etc/slowdns/nameserver
echo $NS_DOMAIN > /etc/slowdns/nsdomain
cd
#install client-sldns.service
cat > /etc/systemd/system/client-sldns.service << END
[Unit]
Description=Client SlowDNS By Rerechan02
Documentation=https://www.rerechan02.com
...
ExecStart=/etc/slowdns/sldns-client -udp 8.8.8.8:5300 --pubkey-file /etc/slowdns/server.pub $nameserver 127.0.0.1:109
Restart=on-failure
...
[Unit]
Description=Server SlowDNS By Rerechan02
Documentation=https://www.rerechan02.com
After=network.target nss-lookup.target
...
AmbientCapabilities=CAP_NET_ADMIN CAP_NET_BIND_SERVICE
NoNewPrivileges=true
ExecStart=/etc/slowdns/sldns-server -udp :5353 -privkey-file /etc/slowdns/server.key $nameserver 127.0.0.1:2222
Restart=on-failure
